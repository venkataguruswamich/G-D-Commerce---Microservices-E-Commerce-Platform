...
export default function Orders() {
  ...
  return (
    <Container className="py-8">
      <h1 className="mb-6 text-h2 font-semibold text-slate-900 dark:text-white">Your Orders</h1>

      {loading && <Loading />}
      <ErrorMessage message={error} />

      {!loading && !error && orders.length === 0 && (
        <EmptyState icon="orders" title="No orders yet" description="Orders you place will show up here." action={{ label: 'Browse Products', to: '/products' }} />
      )}
        ...
        <div className="flex flex-col gap-4">
          {orders.map((order) => (
            <Card key={order.id} padding="md" className="flex flex-wrap items-center justify-between gap-4">
              ...
              <Link to={`/orders/${order.id}`} className="text-small font-medium text-brand-600 hover:text-brand-700 dark:text-brand-400">
                View Details
              </Link>
            </Card>
          ))}
