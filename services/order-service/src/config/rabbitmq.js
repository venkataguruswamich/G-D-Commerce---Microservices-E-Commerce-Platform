...
/**
 * Lazily connects and asserts the shared topic exchange + its dead-letter
 * exchange. Safe to call repeatedly — returns the cached channel once open.
 *
...
/**
 * Declares a durable queue with a dead-letter path and a companion retry
 * queue (TTL-based delay that dead-letters back into the main queue), then
 * binds it to the shared exchange on the given routing patterns.
 */
...
/**
 * Consumes `queueName` with manual ack. On handler failure the message is
 * routed to `${queueName}.retry` (which delays it and redelivers to the
 * original queue) up to `maxRetries` times; after that it is nacked without
 * requeue so the queue's dead-letter config routes it to `${queueName}.dlq`.
